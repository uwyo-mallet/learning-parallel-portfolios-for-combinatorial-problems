cluster = NaN
learnertype = "randomForest"
benchmark_name = "SAT11-INDU"
se.method = ""
combined = "seperate"
plot_title = "SAT11-INDU"
...
assign_jp_optimum = function(learnertype, se.method, combined) {
  ...
  # randomForest
  # jp_optimum = 0.63
  # Define a nested list (acting as a dictionary) with the updated jp_optimum values
  jp_values <- list(
    combined = list(
      ranger = list(
        jack = 0.04,   # Updated value
        infjack = 0    # Updated value
      ),
      randomForest = 0.41 # Updated value
    ),
    seperate = list(
      ranger = list(
        jack = 0.31,   # Updated value
        infjack = 0.01 # Updated value
      ),
      randomForest = 0.63 # Updated value
    )
  )
  
  # Check if the 'combined' input is valid
  if (!combined %in% names(jp_values)) {
    stop("Invalid 'combined' value. Use 'combined' or 'seperate'.")
  }
  
  # Check if the 'learnertype' input is valid
  if (!learnertype %in% names(jp_values[[combined]])) {
    stop(paste("Learnertype", learnertype, "not recognized."))
  }
  
  # Retrieve the jp_optimum value based on the input parameters
  learner_info <- jp_values[[combined]][[learnertype]]
  
  # Determine the jp_optimum value depending on whether a specific se.method is provided
  if (is.list(learner_info) && se.method != "") {
    ...
    jp_optimum <- learner_info # Use this for methods without sub-methods or when se.method is not applicable
  }
...
assign_jp_generic = function(learnertype, se.method, combined) {
  #should be updated:
  ...
  # Define a nested list (acting as a dictionary) with the jp_generic values
  jp_values <- list(
    # combined = list(
    #   ranger = list(
    #     jack = 0.65,
  ...
  if (is.list(learner_info) && se.method != "") {
    ...
    jp_generic <- learner_info[[se.method]]
  } else {
    jp_generic <- learner_info # For methods without sub-methods or when se.method is not applicable
  }
...
# jp_average_optimum = assign_jp_generic(learnertype, se.method, combined)
...
assign_kl_generic = function(learnertype, se.method, combined) {
  kl_values <- list(
    # combined = list(
    #   ranger = list(
    #     jack = 0.65,
  ...
  if (is.list(learner_info) && se.method != "") {
    ...
    kl_generic <- learner_info[[se.method]]
  } else {
    kl_generic <- learner_info # For methods without sub-methods or when se.method is not applicable
  }
  
  return(kl_generic)
}

# kl_average_optimum = assign_kl_generic(learnertype, se.method, combined)
...
set_paths = function(){
  if(Sys.info()['sysname'][[1]]=="Linux"){
    if(file.exists("~/Documents/nextSteps/Scripts/OverheadResults.R")){
      ...
      kl_optimum_avg_path <<- paste0("~/Documents/ranger_results_",combined,"/","/results/",benchmark_name,"/",learnertype, "/", se.method,"/kl_div_avg/")
      create_directory_if_not_exist(kl_optimum_avg_path)
    } else{
      ...
      create_directory_if_not_exist(jp_optimum_avg_path)
      kl_optimum_avg_path <<- paste0("/gscratch/hkashgar/ranger_results_",combined,"/","/results/",benchmark_name,"/",learnertype, "/", se.method,"/kl_div_avg/")
      create_directory_if_not_exist(kl_optimum_avg_path)
    }
    ...
    create_directory_if_not_exist(jp_optimum_avg_path)
    kl_optimum_avg_path <<- paste0("C:/Users/hnyk9/Thesis/ranger_results_",combined,"/","/results/",benchmark_name,"/",learnertype, "/", se.method,"/kl_div_avg/")
    create_directory_if_not_exist(kl_optimum_avg_path)
  }
}
...
get_joint_probability_optimum = function(){
  ...
  for(r in range){
    ...
    # print(summary)
    print(min)
    print(as.numeric(summary[[1]]$Parallel_time))
...
# get_joint_probability_results_averageOpt(TRUE, optimum = jp_average_optimum)



get_kl_div_results_averageOpt = function(saved = TRUE, optimum = kl_average_optimum){
  print("get_kl_div_results_averageOpt")
  print(kl_average_optimum)
  set_paths()
  self = PredictionResults$new(benchmark_name, cluster = cluster, learnertype = learnertype, se.method = se.method)
  range = c(1:max_core)
  resR = data.frame(matrix(ncol= (max_core+1),nrow=0))
  ...
  if(!saved){
    for(i in range){
      saveto = paste0(kl_optimum_avg_path,i, "-core")
      create_directory_if_not_exist(saveto)
      if(combined == "combined"){
        pred_path = paste0(predictionPath,"/preds_combined/")
      } else{ 
...
# get_kl_div_results_averageOpt(FALSE, optimum = kl_average_optimum)
